...
(function() {
	...
	var suduguRule = {
		...
		name: "速读谷",
		version: 1,
		match: { pattern: "^https?://www\\.sudugu\\.org/\\d+/\\d+(?:-\\d+)?\\.html(?:[?#].*)?$" },
		content: {
			selector: ".con",
			remove: "script, style, iframe, ins"
		},
	...
	var AutoEnableManager = class {
		...
		async check(doc = document) {
			...
			if (isCloudflareChallenge(doc)) return decide({
				shouldEnable: false,
				method: "manual",
				confidence: 0,
...
`;
	...
	var toDicts = { cn: cn_default };
	var configs = {
		hk2s: {
			normalizationChain: [[CJK_Compatibility_Ideographs_default]],
			segmentation: [TSPhrases_default],
			conversionChain: [[HKVariantsRevPhrases_default, HKVariantsRev_default], [TSPhrases_default, TSCharacters_default]]
		},
		hk2sp: {
			normalizationChain: [[CJK_Compatibility_Ideographs_default]],
			segmentation: [TSPhrases_default],
			conversionChain: [[
				...
				HKVariantsRevPhrases_default,
				HKVariantsRev_default
			], [TSPhrases_default, TSCharacters_default]]
		},
		t2s: {
			normalizationChain: [[CJK_Compatibility_Ideographs_default]],
			conversionChain: [[TSPhrases_default, TSCharacters_default]]
		},
		tw2s: {
			normalizationChain: [[CJK_Compatibility_Ideographs_default]],
			segmentation: [TSPhrases_default],
			conversionChain: [[TWVariantsRevPhrases_default, TWVariantsRev_default], [TSPhrases_default, TSCharacters_default]]
		},
		tw2sp: {
			normalizationChain: [[CJK_Compatibility_Ideographs_default]],
			segmentation: [TSPhrases_default],
	...
	function ConverterFactoryWithSegmentation(segmentationDict, ...dictGroups) {
		...
		if (segmentationDict) {
			...
			if (Array.isArray(segmentationDict) && segmentationDict.every((dict) => typeof dict === "string")) segmentation.loadDictGroup(segmentationDict);
			else segmentation.loadDict(segmentationDict);
		}
		...
		return function convert(s) {
			...
			return trieArr.reduce((segments, t) => segments.map((segment) => t.convert(segment)), segments).join("");
		};
	}
	function ConverterBuilder(localePreset) {
		function getConfigName(from, to) {
			if (from === "cn") return `s2${to}`;
			if (to === "cn") {
				if (from === "hkp") return "hk2sp";
				return from === "twp" ? "tw2sp" : `${from}2s`;
			}
			return `${from}2${to}`;
		}
		function normalizeDictGroups(dictGroup) {
			if (Array.isArray(dictGroup) && Array.isArray(dictGroup[0])) return dictGroup;
			return [dictGroup];
		}
		return function Converter(options) {
			["from", "to"].forEach((type) => {
				if (!options || typeof options[type] !== "string") throw new Error("Please provide the `" + type + "` option");
				if (options[type] !== "t" && !localePreset[type][options[type]]) throw new Error("Unknown `" + type + "` locale: " + options[type]);
			});
			if (localePreset.configs) {
				...
				if (config) {
					...
					if (!config.normalizationChain) return converter;
			...
			let dictGroups = [];
			["from", "to"].forEach((type) => {
				if (options[type] !== "t") dictGroups.push(...normalizeDictGroups(localePreset[type][options[type]]));
			});
	...
	function getSimplifiedConverter() {
		simplifiedConverter ??= ConverterBuilder(t2cn_exports)({
			from: "t",
			to: "cn"
		});
		...
		async function startCacheAll(urls) {
			...
			if (!taskList.length) {
				...
				if (indexUrl) {
					...
					taskList = tocEntries.map((e) => normalizeUrlForFetch(e.url)).slice(0, 1e4).filter((u) => !ctx.loadedUrls.value.has(u) && !ctx.cachedContents.value.has(u) && !persistedSet.has(u));
					ctx.cacheQueue.value = [...taskList];
				}
			}
	...
	function isVipChapterPage(doc) {
		...
		const cta = normalizeTextForVipDetection(Array.from(doc.querySelectorAll("a,button,input[type=\"button\"],input[type=\"submit\"]")).map((el) => {
	...
	function useKeyboardShortcuts(shortcuts, options = {}) {
		...
		function handleKeyDown(ev) {
			...
			for (const shortcut of shortcuts) {
				...
				if (ignoreInputs && !shortcut.allowInInputs && isInputElement(target)) continue;
	...
	function useReaderScroll(options) {
		...
		return {
			handleScroll: throttle(handleScrollCore, SCROLL_THROTTLE_MS),
			lastScrollTop
		};
	}
	...
	function useReaderAutoLoad(options) {
		const { mainRef, chapterRefs, readerStore, configStore, hasNext, isLoadingNext, isLoadingPrev, isLoading, isNavigating } = options;
		...
		function getRandomDelayMs(min, max) {
			...
			return Math.floor(Math.random() * (Math.max(min, max) - a + 1)) + a;
		}
		function now() {
			return Date.now();
		}
		...
		async function finishLoad(ok, startedSessionKey, startedTailId) {
			...
			if (ok) {
				...
				if (bufferState === "short") graceUntil = now() + 300;
				scheduleAutoLoadNext("state");
				return;
			}
			autoLoadInFlight = false;
		...
		if (typeof window !== "undefined") window.addEventListener("resize", handleResize, { passive: true });
		scheduleAutoLoadNext("state");
		onUnmounted(() => {
			clearAutoLoadTimer();
	...
	function useTouchGestures(options) {
		...
		function handleTouchMove(e) {
			...
			if (Math.abs(dy) >= SWIPE_CANCEL_VERTICAL_PX && Math.abs(dy) >= Math.abs(dx) * SWIPE_AXIS_RATIO) swipeStart.cancelled = true;
		}
	...
	function useChapterNavigation(options) {
		...
		function loadNextAtBoundary() {
			...
			readerStore.loadNextChapter("manual");
		}
		function scrollToChapter(index) {
			const url = chapters.value[index]?.chapter.url;
			if (!url) return;
			const chapterEl = chapterRefs.get(url);
			if (chapterEl) chapterEl.scrollIntoView({
				behavior: "smooth",
				block: "start"
			});
		}
		...
		async function turnReaderPage(direction) {
			...
			if (direction === "next") {
				...
				try {
					...
					if (loaded) await jumpToChapter(readerStore.chapters.length - 1, "auto");
				} finally {
		...
		function handleWheel(e) {
			...
			if (e.deltaY < 0 && isAtTop(mainEl)) {
				...
				if (hasPrev.value && !isLoadingPrev.value && !isNavigating.value) loadPrevWithScrollAdjust();
				return;
			}
	...
	var _hoisted_11$1 = ["aria-valuenow"];
	var _hoisted_12$1 = {
		key: 1,
	...
	var _hoisted_18$1 = { class: "mnr-chapter-title-text" };
			...
			return (_ctx, _cache) => {
				return openBlock(), createElementBlock(Fragment, null, [createVNode(Transition, { name: "mnr-fade" }, {
					...
					createBaseVNode("section", _hoisted_7$3, [
						...
						}, toDisplayString(__props.cacheProgress.running ? "取消" : "缓存本书"), 1)]),
						...
						}, null, 4)], 8, _hoisted_11$1)) : createCommentVNode("", true),
						!__props.cacheProgress.running && (__props.cacheProgress.failed > 0 || __props.persistedCount > 0) ? (openBlock(), createElementBlock("div", _hoisted_12$1, [__props.cacheProgress.failed > 0 ? (openBlock(), createElementBlock("button", {
							key: 0,
					...
					__props.loading ? (openBlock(), createElementBlock("div", _hoisted_13$1, [createVNode(unref(MnrSpinner_default), { size: "small" }), _cache[9] || (_cache[9] = createBaseVNode("span", null, "加载目录中...", -1))])) : __props.chapters.length === 0 ? (openBlock(), createElementBlock("div", _hoisted_14$1, "暂无目录")) : filteredChapters.value.length === 0 ? (openBlock(), createElementBlock("div", _hoisted_15$1, "没有匹配的章节")) : (openBlock(), createElementBlock("div", {
						key: 4,
						ref_key: "contentRef",
						ref: contentRef,
						...
						}, [ch.isPersisted && !ch.isCurrent ? (openBlock(), createElementBlock("span", _hoisted_17$1, [..._cache[10] || (_cache[10] = [createBaseVNode("svg", {
							viewBox: "0 0 24 24",
							"aria-hidden": "true"
						}, [createBaseVNode("path", { d: "m5 12 4 4L19 6" })], -1)])])) : createCommentVNode("", true), createBaseVNode("span", _hoisted_18$1, toDisplayString(ch.title), 1)], 10, _hoisted_16$1)]);
					}), 128))], 4)], 544))
				], 10, _hoisted_1$3)], 64);
			};
		}
	}), [["__scopeId", "data-v-aeddb985"]]);
	...
	var ReaderView_default = _plugin_vue_export_helper_default(defineComponent({
		...
		setup(__props, { emit: __emit }) {
			...
			const { navigateChapter, jumpToCachedChapter, scrollReader, turnReaderPage, handleWheel } = useChapterNavigation({
				mainRef,
				chapters,
				chapterRefs,
				readerStore,
			...
			const { handleTouchStart, handleTouchMove, handleTouchEnd, handleTouchCancel } = useTouchGestures({
				enabled: computed(() => configStore.behavior.swipeGestures && !hasOpenPanel.value && !isLoading.value && !isLoadingPrev.value && !isLoadingNext.value && !isNavigating.value),
			...
			function clearBoundaryPull() {
				boundaryPull = null;
				boundaryGestureHint.value = "";
				boundaryGestureDirection.value = null;
			}
			function isGestureBlocked(event) {
				if (hasOpenPanel.value || isLoading.value || isLoadingPrev.value || isLoadingNext.value || isNavigating.value) return true;
				const selection = window.getSelection();
				if (selection && selection.toString().length > 0) return true;
				if (!(event.target instanceof Element)) return false;
				return Boolean(event.target.closest("a, button, input, textarea, select, label, summary, [contenteditable], [role=\"button\"]"));
			}
			function handleReaderTouchStart(event) {
				clearBoundaryPull();
				const mainEl = mainRef.value;
				if (mainEl && isSingleTouchEvent(event) && !isGestureBlocked(event)) {
					const direction = isAtBottom(mainEl) && hasNext.value ? "next" : isAtTop(mainEl) && hasPrev.value ? "prev" : null;
					if (direction) {
			...
			function guardTouchBoundary(event) {
				...
				if (Math.abs(deltaX) >= BOUNDARY_PULL_HINT_PX && Math.abs(deltaX) > Math.abs(deltaY) * BOUNDARY_AXIS_RATIO) {
					clearBoundaryPull();
					return;
				}
				...
				if (boundaryPull.direction === "next") boundaryGestureHint.value = boundaryPull.ready ? "松手加载下一章" : "继续上滑加载下一章";
				else boundaryGestureHint.value = boundaryPull.ready ? "松手加载上一章" : "继续下滑加载上一章";
			...
			function handleReaderTouchEnd(event) {
				...
				if (boundaryDirection) turnReaderPage(boundaryDirection);
				scheduleAutoLoadNext("settled");
			}
			...
			function navigate(direction) {
				if (direction === "index" && indexUrl.value) window.location.href = indexUrl.value;
			}
			...
			function handleProtectionModeChange(mode) {
				emit("protectionModeChange", mode);
			}
			...
			onMounted(async () => {
				...
				topObserver = new globalThis.IntersectionObserver(() => {}, observerOptions);
			...
			return (_ctx, _cache) => {
				return openBlock(), createElementBlock("div", {
					...
					showProgress.value ? (openBlock(), createBlock(ProgressIndicator_default, {
						key: 0,
						percent: scrollPercent.value,
						"auto-hide": true
					}, null, 8, ["percent"])) : createCommentVNode("", true),
					boundaryGestureHint.value ? (openBlock(), createElementBlock("div", {
						key: 1,
						class: normalizeClass(["mnr-boundary-gesture-hint", `is-${boundaryGestureDirection.value}`]),
						role: "status",
						"aria-live": "polite"
					}, toDisplayString(boundaryGestureHint.value), 3)) : createCommentVNode("", true),
