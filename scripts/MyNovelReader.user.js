...
(function() {
	...
	var DetectionEngine = class {
		...
		detect(doc = document, currentUrl = window.location.href) {
			...
			const results = {
				...
				title,
				section: this.navigationDetector.detectSection(doc, currentUrl, validatedNav)
			};
			return {
				results,
		...
		detectSection(doc = document, currentUrl = window.location.href) {
			const navigation = this.navigationDetector.detect(doc, currentUrl);
	...
	var specialRules = [
		{
			...
			name: "创世中文网",
			version: 1,
			match: { pattern: "^https?://(?:chuangshi|yunqi)\\.qq\\.com/|^http://dushu\\.qq\\.com/read.html\\?bid=" },
			content: { selector: ".bookreadercontent" },
			navigation: {
		...
		},
		{
			...
			name: "微信读书",
			version: 1,
			match: { pattern: "https?://weread\\.qq\\.com/web/reader/.*?" },
			content: { selector: ".wr_canvasContainer" },
			navigation: {
				next: "#nextchapter",
				prev: "#prevchapter",
				index: "#bookindex"
		...
		},
		{
			...
			name: "阿拉法小说网",
			version: 1,
			match: { pattern: "https?://www.alafaxs.com/du/\\d+/\\d+.html" },
			content: { selector: "#txt" },
			title: { bookSelector: ".chapter-nav > p:first > a:last()" },
			advanced: {
				useIframe: true,
		...
		},
		{
			...
			name: "哔哩轻小说",
			version: 1,
			match: { pattern: "https://(www|tw)\\.(bilinovel|linovelib)\\.com/novel/\\d+/\\d+(_\\d+)?\\.html" },
			content: { selector: ".acontent, .bcontent" },
			navigation: {
				next: "#footlink > a:nth-child(4)",
				prev: "#footlink > a:nth-child(1)",
				index: "#footlink > a:nth-child(2)"
			},
			title: { selector: "#atitle" },
			advanced: {
				useIframe: true,
				checkSection: true
			},
		...
		},
		{
			...
			name: "69shux",
			version: 1,
			match: { pattern: "https://69shux.com/txt/\\d+/\\d+" },
			content: {
				selector: ".txtnav",
	...
	var simplifiedRules = [
		{
			...
			name: "我的书城网",
			version: 1,
			match: { pattern: "^https?://www\\.wodeshucheng\\.net/.*?\\.html$" },
			content: {
				selector: "#content",
		...
		},
		{
			...
			version: 1,
			match: { pattern: "^https?://(?:www\\.)?uukanshu\\.cc/book/\\d+/\\d+\\.html(?:\\?.*)?$" },
			content: { selector: ".readcotent, #contentbox, #content, #chaptercontent, #chapter_content, .content" },
			meta: {
				source: "builtin",
		...
		},
		{
			...
			name: "纵横中文网",
			version: 1,
			match: { pattern: "^https?://book\\.zongheng\\.com/\\S+\\/\\d+\\.html$" },
			content: {
				selector: "#readerFt",
				remove: ".watermark"
			},
		...
		},
		{
			...
			name: "纵横中文网-read",
			version: 1,
			match: { pattern: "https?://read\\.zongheng\\.com/chapter/\\d+\\/\\d+\\.html" },
			content: {
				selector: ".content",
				remove: ".Jfcounts"
			},
			title: { selector: ".title_txtbox" },
			meta: {
				source: "builtin",
		...
		},
		{
			...
			name: "晋江文学城_手机版",
			version: 1,
			match: { pattern: "^https?://(?:wap|m)\\.jjwxc\\.(?:net|com)/(?:book2|vip)/\\d+/\\d+" },
			content: { selector: "div.grid-c > div > .b.module > div:first" },
			title: {
				selector: "h2",
				pattern: "《(.*?)》.*[ˇ^](.*?)[ˇ^].*"
			},
		...
		},
		{
			...
			name: "潇湘书院",
			version: 1,
			match: { pattern: "^https?://www\\.xxsy\\.net/chapter/.*\\.html" },
			content: {
				selector: "#auto-chapter",
				replace: [{
					pattern: "本书由潇湘书院首发，请勿转载！",
					replacement: ""
				}]
			},
			navigation: {
		...
		},
		{
			...
			name: "逐浪",
			version: 1,
			match: { pattern: "^https?://book\\.zhulang\\.com/.*\\.html" },
			content: { selector: "#readpage_leftntxt" },
			title: { pattern: "(.*?)-(.*)" },
			processing: { removeAds: false },
			meta: { source: "builtin" }
		},
		{
			...
			name: "小说阅读网",
			version: 1,
			match: { pattern: "^https?://www\\.readnovel\\.com/novel/.*\\.html" },
			content: {
		...
		},
		{
			...
			name: "百度贴吧（手动启用）",
			version: 1,
			match: { pattern: "^https?://tieba\\.baidu.com/p/" },
			content: {
		...
		},
		{
			...
			name: "17k小说网",
			version: 1,
			match: { pattern: "^https?://\\S+\\.17k\\.com/chapter/\\S+/\\d+\\.html$" },
			content: {
		...
		},
		{
			...
			name: "书海小说",
			version: 1,
			match: { pattern: "^https?://www\\.shuhai\\.com/read/\\d+/\\d+\\.html" },
			content: {
		...
		},
		{
			...
			name: "露西弗俱乐部",
			version: 1,
			match: { pattern: "^https://www\\.lucifer-club\\.com/.*\\.html" },
			content: {
				selector: "#luf_news_contents",
				remove: "> form, #luf_local, .luf_top_ad, .luf_news_title, .luf_page_control, .luf_comment",
				replace: [{
					pattern: "保护版权 尊重作者 @ 露西弗俱乐部 www\\.lucifer-club\\.com",
					replacement: ""
				}]
			},
			navigation: { index: ".luf_news_title > a:contains(\"目录\")" },
			title: { bookSelector: "#luf_local > a:nth-child(3)" },
			processing: { removeAds: false },
			meta: {
				source: "builtin",
		...
		},
		{
			...
			name: "真白萌",
			version: 1,
			match: { pattern: "^https?://masiro\\.me/admin/novelReading" },
			content: { selector: ".nvl-content.box-body" },
			navigation: {
				next: "a:contains('下一话')",
				prev: "a:contains('上一话')"
			},
		...
		},
		{
			...
			name: "123读",
			version: 1,
			match: { pattern: "https?://www\\.123dua?\\.(com|vip)/dudu-\\d+/\\d+/\\d+(-\\d+)?\\.html" },
			content: { selector: "#content" },
			navigation: { next: "#PageSet a:contains(\"下一页\"), .bottem2 a:contains(\"下一章\")" },
			advanced: { checkSection: true },
			meta: { source: "builtin" }
		},
		{
			...
			name: "独步小说网",
			version: 1,
			match: { pattern: "https?://www.dbxsc.com/book/.*?/.*?\\.html" },
			content: { selector: "#cont-body" },
			navigation: {
		...
		},
		{
			...
			name: "奇书网",
			version: 1,
			match: { pattern: "https://www.qisxs.com/.*?/\\d+.html" },
			content: { selector: ".box_box" },
			title: { bookSelector: ".info a" },
			meta: {
				source: "builtin",
		...
		},
		{
			...
			name: "622中文",
			version: 1,
			match: { pattern: "https://www.622zw.com/books/\\d+/\\d+(_)?\\d+.html" },
			content: { selector: "#content" },
			title: { selector: ".reader-main h1.title" },
			advanced: { checkSection: true },
			meta: {
				source: "builtin",
		...
		},
		{
			...
			name: "逛笔趣阁小说网",
			version: 1,
			match: { pattern: "https?://www\\.fkxs\\.net/.*?/.*?\\.html" },
			content: { selector: ".content" },
			navigation: {
		...
		},
		{
			...
			name: "永久看小说",
			version: 1,
			match: { pattern: "https://www.(09k|09kan).net/kkb/\\d+/\\d+(-\\d+)?.html" },
			content: { selector: "#content" },
			navigation: {
		...
		},
		{
			...
			name: "语录书院",
			version: 1,
			match: { pattern: "https://www.yulusy.com/yulus/\\d+/.*?.html" },
			content: { selector: "#content" },
			navigation: {
				next: "#PageSet a:contains('下'):contains('页')",
				prev: "#PageSet a:contains('上'):contains('页')"
		...
		},
		{
			...
			name: "书海阁小说网(手机版)",
			version: 1,
			match: { pattern: "^https?://m\\.shuhaige\\.net/\\d+/\\d+(?:_\\d+)?\\.html$" },
			content: {
				selector: ".content",
				replace: [{
	...
	var SectionMerger = class {
		...
		async merge(doc, url, options = {}) {
			...
			if (!!first.rule?.advanced?.noSection) return first;
			const enableByRule = !!first.rule?.advanced?.checkSection;
			const section = this.parser.detect(startDoc, startUrl).results.section;
			...
			if (!(enableByRule || !!section?.isSection && (section?.confidence || 0) >= confidenceThreshold)) {
				if (!hasNextSectionUrl && first.nextUrl && isSectionLikeUrl(startUrl, first.nextUrl)) {
					...
					if (realNextChapterUrl) first.nextUrl = realNextChapterUrl;
				}
				return first;
			}
			const sectionDelayMs = options.fetcher ? 0 : Math.max(0, first.rule?.advanced?.sectionDelayMs ?? 0);
			return this.mergeSections(startUrl, first, section, maxPages, sectionDelayMs, options.fetcher, knownDocs, options.signal);
		}
		async mergeSections(startUrl, first, section, maxPages, sectionDelayMs, fetcher, knownDocs, signal) {
			...
			if (!nextSectionUrl && first.nextUrl && isSectionLikeUrl(startUrl, first.nextUrl)) nextSectionUrl = first.nextUrl;
			...
			for (let i = 0; i < maxAdditionalPages && nextSectionUrl; i++) {
				...
				const s = this.parser.detect(nextDoc, absNextSection).results.section;
				...
				if (!nextSectionUrl && nextParsed.nextUrl) {
					if (isSectionLikeUrl(absNextSection, nextParsed.nextUrl)) nextSectionUrl = nextParsed.nextUrl;
					else if (!nextChapterUrl) nextChapterUrl = nextParsed.nextUrl;
				}
...
`;
	...
	function useReaderScroll(options) {
		const { mainRef, chapters, visibleChapters, chapterRefs, chapterHeights, averageHeight, setChapterHeight, updateWindow, readerStore, autoHideHeader, showControls, isNavigating, scheduleAutoLoadNext } = options;
		let lastScrollTop = 0;
