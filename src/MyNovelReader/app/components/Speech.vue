<template>
  <div class="speech">
    ...
    <span v-else>
      <button v-if="playState == STATE.stoping" @click="start">开始朗读</button>
      <button v-if="playState == STATE.pausing" @click="resume">继续朗读</button>
    ...
    <div v-else>
      <div>
        <label
          ><input v-model="autoStop" type="radio" value="time" />
          定时朗读：
          <input v-model="autoStopTime" type="text" class="auto-stop-input" />
        ...
        ><br />
        <label
          ><input v-model="autoStop" type="radio" value="chapter" />
          定章朗读：
          <input v-model="autoStopChapter" type="text" class="auto-stop-input" />章后停止 </label
        ><br />
        <label><input v-model="autoStop" type="radio" value="" />一直朗读</label>
      </div>
      <div>
        <label for="speech-dialog-rate">语速</label>
        <input id="speech-dialog-rate" v-model="rate" type="range" min="0.5" max="3" step="0.1" />
        <span class="rate-value">{{ rate }}</span>
      </div>
      <div>
        <label for="speech-dialog-pitch">音高</label>
        <input id="speech-dialog-pitch" v-model="pitch" type="range" min="0" max="2" step="0.1" />
        <span class="pitch-value">{{ pitch }}</span>
      </div>
      <div>
        <select v-model="selectedVoice" class="voices">
          <option v-for="(voice, index) in voiceList" :key="index" :value="index">
