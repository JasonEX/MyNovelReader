...
const sites = [
  // 详细版规则示例。注：该网站已无法访问。
  {
    ...
    contentReplace:
      /(\*W|（w|\(w).{10,25}(吧\*|）|\))|看小说就上|本书首发|泡.{1,6}吧|百度搜索阅读最新最全的小说|http:\/\/www.paoshu8.com\/|无弹窗/g, // 需要移除的内容正则 (可选)
    contentPatch: function (fakeStub) {
      // (可选)内容补丁。解决翻页是脚本的情况
      var $next = fakeStub.find('#LinkMenu');
      $next.html(
        $next.html().replace(/<script>ShowLinkMenu.*?(<a.*?a>).*?(<a.*?a>).*?script>/, '$1$2') +
  ...
  },
  {
    ...
    contentPatch: function ($doc) {
      ...
      if ($doc.find('#j_chapterPrev').length === 0) {
        ...
        // 加上一页链接
        $('<div id="j_chapterPrev">').attr('href', prevUrl).appendTo($doc.find('body'));
        // 加下一页链接
        $('<div id="j_chapterNext">').attr('href', nextUrl).appendTo($doc.find('body'));
        // 目录
        var indexUrl = $('#bookImg').attr('href') + '#Catalog';
        $('<div id="my_index">目录</div>').attr('href', indexUrl).appendTo($doc.find('body'));
      }
    },
    startLaunch($doc) {
      const win = $doc[0].defaultView;
      if (win.g_data.chapter.vipStatus === 1) {
        // 是 vip 章节
        this.useiframe = true;
        this.mutationSelector = '.read-content.j_readContent';
        this.mutationChildCount = 0;
      }
      if (win.g_data.chapter.cES === 2) {
        // vip 加密 + Html、Css 混淆章节
        // 不支持
  ...
  },
  {
    ...
    getContent: function ($doc, callback) {
      ...
      function getPageUrlHtml(preChapterUUID, nextChapterUUID) {
        ...
        return (
          '<a id="rightFloatBar_preChapterBtn" href="' +
          preReadUrl +
          '">上一页</a>' +
          '<a id="rightFloatBar_nextChapterBtn" href="' +
          nextReadUrl +
          '">下一页</a>' +
          '\n'
        );
      ...
      unsafeWindow.CS.page.read.main.getChapterContent(
        unsafeWindow.bid,
        unsafeWindow.uuid,
  ...
  },
  {
    ...
    contentPatch: function (fakeStub) {
      ...
      fakeStub.find('title').text(fakeStub.find('.tc > h1').text() + '-' + chapterTitle);
    },
  },
  {
    siteName: '纵横中文网',
  ...
  },
  {
    ...
    contentReplace: ['@无限好文，尽在晋江文学城'],
    startLaunch($doc) {
      const win = $doc[0].defaultView;
      if (win.location.href.includes('onebook_vip')) {
        // vip 章节
        this.useiframe = true;
  ...
  },
  {
    ...
    contentPatch: function (fakeStub) {
      var title =
        fakeStub.find('.readpage_leftnzgx a:first').text() +
        '-' +
  ...
  },
  {
    ...
    style:
      '.clear { border-top:1px solid #cccccc; margin-bottom: 50px; visibility: visible !important;}', // 显示楼层的分割线
  },
  {
    siteName: '17k小说网',
  ...
  // 上下页链接难搞
  {
    ...
    contentReplace: ['来源长佩文学网（https://www.gongzicp.com）'],
  },
  // ===========================================================
  {
    siteName: 'E品中文网',
  ...
  },
  {
    ...
    contentReplace: [
      /[{〖]请在百度搜索.*[}〗]|.(?:百度搜索飄天|无弹窗小说网).*\.Net.|\[飄天.*无弹窗小说网\]/gi,
  ...
  },
  {
    ...
    contentHandle: false,
    contentRemove:
      '> form, #luf_local, .luf_top_ad, .luf_news_title, .luf_page_control, .luf_comment',
  ...
  },
  {
    ...
    contentReplace: ['欢迎访问英文小说网http://novel.tingroom.com'],
  },
  {
  ...
  // 这网站为了防抓取，内容顺序都是不对的，只好采用 iframe 方式
  {
    ...
    // 后面的是 和图书 的干扰码
    contentRemove:
      'h2, acronym, bdo, big, cite, code, dfn, kbd, q, s, samp, strike, tt, u, var, ins',
    contentPatch: function ($doc) {
      // 转换 div 到 p
      $doc.find('#content div').each(function () {
        let html = $(this).html();
        $(this).replaceWith($('<p>').html(html));
      });
    },
  },
  {
  ...
  },
  {
    ...
    contentPatchAsync: async function ($doc) {
      ...
      function encode(plainText) {
        if (
          ('number' == typeof plainText && (plainText = plainText.toString()),
          'string' != typeof plainText)
        )
          return plainText;
        ...
        for (
          var _0x59bebd = _0x2045c1[1], _0x1aafd3 = 0;
          _0x1aafd3 < _0x59bebd.length;
          _0x1aafd3++
        ) {
      ...
      function decode(encodedText) {
        ...
        for (
          var _0x25eace = encodedText.charAt(3),
            ...
            _0x2bf2ee = _0x6aad1d;
          _0x2bf2ee < _0x23fa2c &&
          (_0x2bf2ee === _0x6aad1d || 'g' === encodedText.charAt(_0x2bf2ee));
        ) {
          ...
          if ('3' === _0x25eace) {
            ...
            for (
              var _0x293868 = 0, _0x14b43a = _0x3f72db.length;
              _0x293868 < _0x14b43a;
              _0x293868 += 2
            ) {
  ...
  },
  {
    ...
    contentReplace: [
      '.*www\\.xs321\\.net.*',
      '本章未完，请点击下一页继续阅读！',
      '^.*?提示您：看后求收藏（小说321xs321.net），接着再看更方便。',
      '.*xs321.net.*',
    ],
  },
  {
  ...
  },
  {
    ...
    contentPatch($doc) {
      ...
      const hostIP = document.cookie.replace(/(?:(?:^|.*;\s*)hostip\s*\=\s*([^;]*).*$)|^.*$/, '$1');
  ...
  },
  {
    ...
    useiframe: true,
    contentSelector: '.con',
    contentReplace: [{ II: '二' }, { 壹: '一' }, { 贰: '二' }, { Ⅱ: '二' }],
  },
  {
    siteName: '逛笔趣阁小说网',
  ...
  },
  {
    ...
    async getContent($doc) {
      ...
      const { next_chapter, next_chapter_order, chapter_ispay, chapter_name, content } =
        await this.info._getChapterData();
      if (chapter_ispay === 1) {
        this.info.isVipChapter = () => true;
        return;
      }
  ...
  },
  {
    ...
    contentPatch($doc) {
      ...
      $doc
        ...
        .filter(function () {
          return (
            this.nodeType === 3 && new RegExp(_title.replace(/\s+/g, '\\s+')).test($(this).text())
          );
        })
        .remove();
    },
  },
  {
    ...
    contentPatch($doc) {
      ...
      const urls = scriptText
        .match(/url_(previous|index|articleinfo|next):'([^']+)'/g)
        .map(url => url.split(':')[1].slice(1, -1));
