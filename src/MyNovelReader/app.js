...
import { C, L_getValue, L_setValue, L_removeValue, parseHTML, toRE, $x } from './lib';
import UI from './UI';
...
var App = {
  ...
  init: async function () {
    if (['mynovelreader-iframe', 'superpreloader-iframe'].indexOf(window.name) != -1) {
      // 用于加载下一页的 iframe
      return;
    }
  ...
  launch: async function () {
    ...
    // 初始化, 取消页面限制等
    if (App.site.fInit) App.site.fInit();
  },
  ...
  prepDocument: function () {
    ...
    $('html, body').removeAttr('style').removeAttr('bgcolor');

    $('style:not(.noRemove)')
      .filter(function () {
        var $style = $(this);
        if ($style.text().indexOf('#cVim-link-container') != -1) {
          // chrome 的 cVim 扩展
          return false;
        }
        return true;
  ...
  clean: function () {
    $(
      'body > *:not("#container, .readerbtn, .noRemove, #reader_preferences, #uil_blocker,iframe[name=\'mynovelreader-iframe\']")'
    ).remove();
  ...
  appendPage: function (parser, isFirst) {
    ...
    if (chapter.length && parser.isSection) {
      ...
      chapter = $('<article>')
        ...
        .append($('<h1>').addClass('title').text(parser.chapterTitle))
  ...
  resetCache: function () {
    ...
    App.scrollOffsets = App.scrollItems
      .map(function () {
  ...
  registerControls: function () {
    // 内容滚动
    var throttled = _.throttle(App.scroll, 250);
  ...
  pauseHandler: async function () {
    ...
    if (App.paused) {
      UI.notice(
        '<b>状态</b>:自动翻页<span style="color:red!important;"><b>暂停</b></span>'.uiTrans()
      );
      App.$loading
        .html('自动翻页已经<span style="color:red!important;"><b>暂停</b></span>'.uiTrans())
        .show();
    } else {
      UI.notice(
        '<b>状态</b>:自动翻页<span style="color:red!important;"><b>启用</b></span>'.uiTrans()
      );
      await App.scroll();
    }
  },
    ...
    if (nextUrl && !App.isTheEnd && !(nextUrl in App.parsedPages)) {
      ...
      App.$loading
        ...
        .append(
          "<a href='" +
            nextUrl +
            "' title='点击打开下一页链接' tabindex='-1'>正在载入下一页".uiTrans() +
            (useiframe ? '(iframe)' : '') +
            '...</a>'
        );
  ...
  addNextPage: async function (parser) {
    if (parser.content) {
      ...
      var msg = parser.isTheEnd == 'vip' ? 'vip 章节，需付费。' : '错误：没有找到下一页的内容。';
      App.$loading
        .html('<a href="' + App.curPageUrl + '">' + msg + '点此打开下一页。</a>'.uiTrans())
        .show();
    }
  },
