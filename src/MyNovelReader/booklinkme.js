...
var BookLinkMe = {
  ...
  addUnreadButton: function () {
    // 添加一键打开所有未读链接
    var $parent = $(
      'td[colspan="2"]:contains("未读"):first, td[colspan="2"]:contains("未讀"):first'
    );
    ...
    var openAllUnreadLinks = async function (event) {
      ...
      var links = $x(
        './ancestor::table[@width="100%"]/descendant::a[img[@alt="未读"]]',
        event.target
      );
      for (let link of links) {
        ...
        if (isFirefox) link.click();
        else GM_openInTab(link.href);
  ...
  load: function (doc) {
    ...
    var findUrl = function (sTitle) {
      ...
      $data.each(function (i, item) {
        ...
        if (
          tiebaTitle.indexOf(trimTitle(sTitle)) > 0 ||
    ...
    $('a:contains("搜索本章节")').each(function () {
      ...
      if (url) {
        $('<a>').attr({ target: '_blank', href: url }).text('贴吧').appendTo($this.parent());
      }
    });
  },
};
