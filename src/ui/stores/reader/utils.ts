/**
 * Utility functions for reader store
 *
 * Contains helper functions for URL normalization, VIP detection,
 * TOC detection, and network requests.
 */
...
// Local exports for specific store logic below...
// (normalizeUrlForBlock, isVipChapterPage, etc. remain here as they are specific to reader logic)
...
/**
 * Check if a page is a VIP/locked chapter page
 */
export function isVipChapterPage(doc: Document): boolean {
  ...
  // Extra: check common CTA buttons
...
/**
 * Check if a URL is likely not a valid chapter page
 */
...
/**
 * Detect if content is a TOC/table of contents page
 */
export function detectTocPage(
  ...
  const normalizeUrl_ = (url: string) => {
    try {
  ...
  const currentPath = normalizeUrl_(currentChapterUrl);
  const hasLinkToCurrentChapter = Array.from(links).some(a => {
    const href = a.getAttribute('href');
    if (!href) return false;
    return normalizeUrl_(href) === currentPath;
  });
